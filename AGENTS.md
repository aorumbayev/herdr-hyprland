...
herdr-hyprland: Hyprland-style pane/tab/workspace keys as a herdr plugin. One Go module, one binary (`cmd/herdr-hyprland`).
...
- `internal/movetab` — resolve move-to-tab targets; verify tabs still exist.
- `cmd/herdr-hyprland` — `setup`, `move-to-tab N`.
- Native herdr actions are config only. Go is only for move-to-tab.
