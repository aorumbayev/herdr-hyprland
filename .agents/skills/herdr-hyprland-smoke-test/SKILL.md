...
- `tmux send-keys -t hhl-sandbox M-Left` presses `alt+left`. Alt chords are
  ...
  layout); `alt+ctrl+left` is `C-M-Left`.
- `alt+ctrl+tab` has no legacy terminal encoding. Inside tmux it only
  arrives with the kitty keyboard protocol end to end; expect it NOT to work
  in the sandbox and test the last-tab action via
  `hsb herdr plugin action invoke herdr-hyprland.last-tab` instead.
...
- Plugin state: `cat /tmp/hhl-sandbox/state/herdr/plugins/herdr-hyprland/lasttab.json`.
...
- The plugin's event hook fires in every session of a server, and
  `HERDR_PLUGIN_STATE_DIR` in the sandbox points at sandbox state, so the
  user's real lasttab.json is untouched.
